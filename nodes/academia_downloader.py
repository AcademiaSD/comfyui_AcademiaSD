...
def get_headers_with_auth(url, civitai_token=""):
    req_headers = HEADERS.copy()
    if "civitai.com" in url and civitai_token:
        req_headers["Authorization"] = f"Bearer {civitai_token}"
    return req_headers

def get_filename_from_url(url, civitai_token=""):
    # Seguridad: Bloquear peticiones vacías o fantasmas de ComfyUI
    if not url or not url.startswith(('http://', 'https://')):
        return None
        
    try:
        req_headers = get_headers_with_auth(url, civitai_token)
        response = requests.get(url, stream=True, allow_redirects=True, headers=req_headers, timeout=10)
        
        if "text/html" in response.headers.get("Content-Type", "") and "civitai.com" in url:
            print("[AcademiaSD] ⚠️ Warning: Civitai is blocking this download. It requires an API Key.")
            return None
        ...
        print(f"[AcademiaSD] Error resolving URL {url}: {e}")
        return None
...
def background_download_task(url, file_path, civitai_token=""):
        ...
        if match:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    files = [{"name": s["rfilename"], "url": f"https://huggingface.co/{repo_id}/resolve/{branch}/{s['rfilename']}"} for s in siblings if s["rfilename"].endswith(valid_exts)]
...
async def check_file(request):
    ...
    civitai_token = data.get("token", "").strip()
    
    if not url or url == "none" or not url.startswith(('http://', 'https://')):
        return web.json_response({"status": "error", "exists": False})

    # MEGA OPTIMIZACIÓN: Si ya se está descargando, responder al instante y NO molestar a Civitai.
    ...
    filename = await asyncio.to_thread(get_filename_from_url, url, civitai_token)
    if not filename:
        return web.json_response({"status": "error", "exists": False, "message": "auth_required"})
...
async def download_file(request):
    ...
    asyncio.create_task(asyncio.to_thread(background_download_task, url, file_path, civitai_token))
